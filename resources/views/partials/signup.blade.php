<div class="w-full h-screen flex">
    ...
    <div class="flex-1 rightside bg-[#23222E] flex flex-col items-center justify-center">
        <div class="text-white text-5xl font-regular mb-8">I already have an account...</div>
    ...
    <div class="flex-1 flex items-center justify-center h-full">
        <div class="w-full max-w-140 mx-auto ">
            <div class="text-6xl font-black mb-10 mt-8 text-black flex flex-col items-center justify-center">Create Account</div>
             @if ($errors->any() && request()->is('signup'))
                <div class="mb-4 text-red-500 text-sm flex flex-col items-center">
                    @foreach ($errors->all() as $error)
                        <div>- {{ $error }}</div><br>
                    @endforeach
                </div>
            @endif
            <form class="flex flex-col gap-7" method="POST" action="/signup">
                @csrf
                <input type="text" name="username_signup" id="username_signup" autocomplete="off" placeholder="Username" required
                ...
                <div class="relative">
                    ...
                    <button type="button" id="togglePassword-signup" tabindex="-1"
                        class="absolute right-4 top-1/2 -translate-y-1/2 cursor-pointer">
                        <svg id="eyeClosed-signup" width="24" height="24" viewBox="0 0 48 48" fill="none"
                        ...
                        <svg id="eyeOpen-signup" width="24" height="24" viewBox="0 0 48 48" fill="none"
                ...
                <button type="submit"
                    class="bg-orange-400 text-white font-bold text-lg rounded-3xl py-4 mt-2 mb-1 transition active:scale-98 active:bg-orange-500 hover:scale-101">Sign Up</button>
            </form>
        </div>
    </div>
</div>